...
def batch_quotes_period(tickers_tuple, period="1mo"):
    """Price + change vs start of selected period."""
...
def batch_quotes(tickers_tuple):
    """Day-over-day (used only for macro snapshot)."""
    tickers = list(tickers_tuple)
    result  = {t: dict(price=0.0, chg=0.0, pct=0.0) for t in tickers}
    if not tickers: return result
    try:
...
def period_buttons(tab_key, col_offset=0):
    """Render 1W / 1M / 1Y buttons + More dropdown. Returns current period."""
    cols = st.columns([1,1,1,1,2,6])
    with cols[0+col_offset]:
        lbl = "[ 1 Week ]" if st.session_state[tab_key] == "5d" else "1 Week"
        if st.button(lbl, key=f"{tab_key}_5d"):
            st.session_state[tab_key] = "5d"; st.rerun()
    with cols[1+col_offset]:
        lbl = "[ 1 Month ]" if st.session_state[tab_key] == "1mo" else "1 Month"
        if st.button(lbl, key=f"{tab_key}_1mo"):
            st.session_state[tab_key] = "1mo"; st.rerun()
    with cols[2+col_offset]:
        lbl = "[ 1 Year ]" if st.session_state[tab_key] == "1y" else "1 Year"
...
with t5:
    ...
    cp5 = period_buttons("chart_period")
    if ticker:
